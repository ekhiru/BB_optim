...
for instance in $INSTANCES; do
    counter=$((counter+1))
    RESULTS="$OUTDIR/results/m${m}-er${er}-dist_${distance}/$instance"
    mkdir -p "$RESULTS"
    $LAUNCHER umm "${RESULTS}/umm-${init}-learn_${learning}-samp_${sampling}" $instance --m_ini $umm_m_ini --budget $m --init $init --eval_ranks $er --distance $distance --learning $learning --sampling $sampling
