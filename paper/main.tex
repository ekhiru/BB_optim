...
When we compare the behaviour of CEGO and uMM on synthetic LOP instances (Fig.~\ref{fig:lop_synth}).

 we 
: LOP syntéticas, todas en una gráfica. 

Fig.~\ref{fig:loplib}: LOP reales: he puesto 3 de diferentes tamanos. Me ha parecido interesante que, a pesar de que CEGO parece que va mejor, esta diferencia se va reduciendo segun se aumenta el tamano del problema (que se puede ver en el nombre del problema y en el títutlo de la gráfica)

kra32 es el unico del QAP en el que hay un comportaiento no random (considera distancia euclidea) Fig~\ref{fig:kra32}

PFSF, rec05, en un problema muy pequeno ninguno hace nada Fig.~\ref{fig:rec05}
