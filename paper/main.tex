...
Distributions over permutations~\cite{critchlow91} are functions that assign a probability value  $\Prob(\sigma)\in[0,1]$ to each permutation $\sigma \in S_n$. One of the most popular distributions is the Mallows Model (MM), which is considered as an analogous to the Gaussian distribution for permutations. The MM defines the probability of each permutation $\sigma\in S_n$ as follows:\MANUEL{X Could you check this? I wanted to introduce the notation $MM(\sigma_0, \theta)$ that we use later}
%
...
\begin{algorithm}[t]
 ...
 \label{alg:umm}
  \begin{algorithmic}[1]
    ...
    \State decrease on $\theta$\label{line:theta}
...
(\emph{learning}) of the parameters $\sigma_0$ $\theta$ and $\rho$ (line~\ref{line:uborda} in Algorithm~\ref{alg:umm}).\MANUEL{X Is
...
\subsubsection{Learning step.}\ekhine{ }{conceptuaplemet sampling va después}
\ekhine{As explained above, the parameter $\theta$ is updated deterministically at the
sampling step.}{At each iteration we start by fitting a MM to the sample $S$, which implies estimating $\hat\sigma_0$ and $\theta$. $\theta$ is updated deterministically at the sampling step.} 
The parameter $\sigma_0$, however, is learned from the current
sample $S$ by using the uBorda algorithm with a weight
$w(\sigma)=\rho^{f(\sigma)}$ (Section~\ref{sec:uborda}). The learning rate
is controlled by parameter $\rho \in (0,1)$. \ekhine{, which currently is user-defined. Higher values or $\rho$ bias the sampling towards the best solutions found so far, which may lead to quick stagnation, whereas smaller values make the sampling more uniform, thus slowing down the learning. }{}
In this paper, the value of $\rho$ is set at each iteration and depends on the set of function evaluations $F$. Parameter $\rho$ is set such that the largest $r_1$\% of the mass of the weights is concentrated in the best $r_2$\% of the solutions in $S$, i.e., 
\begin{equation}
\begin{split}
...
Sampling from $MM(\sigma_0,\theta)$  can be done efficiently~\cite{IruCalLoz2016permallows} given the parameters  $\sigma_0$ and $\theta$, as studied by \citet{Collas,IruCalLoz2016permallows,Regenwetter2004}\MANUEL{Which paper is Collas? It is not found.}\MANUEL{You call this Regenwetter, but the author is Doignon? \ekhine{}{he is another coauthor} But the entry shows only 1 author.} where the computational complexities are $O(n\log n)$ and $O(n^2)$.\MANUEL{respectively?}
%
At each iteration of UMM, $\sigma_0=\hat\sigma_0$ is the permutation obtained as a solution in the previous learning step. The scale $\theta$ controls the variance and expected distance of the MM $\Edist$, i.e., the distance of a random permutation $\sigma\sim MM(\sigma_0, \theta)$ to the location parameter $\sigma_0$.
\MANUEL{X I'm not sure what is $D$, distance from where to where?} It is set automatically by the algorithm to control the diversification and intensification trade off. At the first iteration, it is set so that the expected distance $\Edist$ equals  $\binom{n}{4}$ (half of the distance of the uniform distribution), and then the expected distance linearly decrease until the last iteration where the expected distance equals 1. Despite it is not possible to isolate $\theta$ in the $\Edist$ expression, Equation~\eqref{eq:expectation}, its monotonicity allows the use of bisection methods for efficient solution approximations. \MANUEL{X I don't understand this sentence, sorry. You say it does not have a closed-form, but you show a closed-form. Do you mean that the formula below is only an approximation of $\Edist$? What is low-cost here? what is high-quality?}\MANUEL{X What is $k$ in the equation?}
%
\begin{equation}\label{eq:expectation}
\Edist = \frac{n \cdot \exp(-\theta)}{1-\exp(-\theta)} - \sum_{j=1}^{n}  \frac{j \exp(-j \theta)}{1-\exp(-j\theta)}
\end{equation}


A purely explotation approach would always generate $\sigma_0$ in the sampling stage.\MANUEL{X I'm not sure what ``systematically generate $\sigma_0$'' means in this context.} However, the setting of a decreasing variance \MANUEL{X which setting? $\theta$?}  controls the exploration-explotation tradeoff as a function of the number of function evaluations that the algorithm performs.  Similar approaches have been taken in different contexts~\cite{arza2019approaching}.\MANUEL{there is something wrong with this reference\ekhine{}{en mi mendeley esta bien :s que puede ser?} }
...
measures~\citep{ZaeStoBar2014:ppsn}. In all experiments, we consider a maximum budget of $\FEmax=400$ evaluations of
...
The first part of the experimental analysis concerns the tunning of parameter $\rho$. As stated in Section~\ref{sec:sampling}, $\rho$ is automatically set at each iteration in such a way that the 100$r_1\%$ of the samples have the 100$r_2\%$ of the weight. In this regard, we tried different configurations of parameters $r_1$ and $r_2$ in all the instances. Figure~\ref{fig:heatmaps} shows the best solution (averaged for 10 runs) for instances: \texttt{N-t1d100.01} and \texttt{N-t1d150.01} for the PFSP, \texttt{reC05} for the PFSP, \texttt{kra32} for the QAP. 
...
Overall the instances we see that the configuration $(r_1, r_2)=(0.1, 0.9)$ has good performance. Therefore, for the lack consistency along the experimental part, we use this configuration. Moreover, we recommend this configuration as a first approach with UMM. 

Moreover, we can see that the behavior of the  two left figures differs from the two in the right: while the two in the left appear to be increase homogeneously from the top right corner to the bottom left one, the two in the right appear to be closer to randomness. We point out that this performance is related to the adequacy of UMM to the problem: homogeneousness is associated to the problem (not to the particular instances) and the more homogeneous the plot, the better performance of UMM. We elaborate this argument in the following sections with detail. 
