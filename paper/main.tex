...
short budgets. Finally, we note that learning Budget-constrained Combinatorial is a hot topic for a variety of structures besides permutation~\cite{manchanda2020}. 
...
exists a wide body of research on probabilistic models for permutations, such as Mallows~\cite{gMallows} that have already been 
been used in optimization problems successfully,
with the most prominent examples being estimation of distribution algorithms
(EDAs)~\cite{Ceberio2013}, \cite{Ceberio2015}. ACO may also be considered a type of EDA, since
it builds a probability distribution model from which solutions are sampled
with a bias towards the best solutions evaluated so far. This works fall into at least one of these assumptions: function evaluations are cheap or the function evaluation is a white-box.


The proposed UMM drastically differs from EDAs since EDAs assume that function
evaluations are cheap and they can do many of them. On the contrary, UMM aims
at learning as much as possible with a limited number of function
...
iteration. Finally, the balance between intensification and exploration is
dynamically adapted by controlling the variance of the probabilistic Mallows
model.

The proposed UMM is based on uBorda~\cite{irurozki2020}, a generalization of
the well-known rank aggregation algorithm Borda~\cite{Borda1781}. Rank
aggregation algorithms summarize in a single permutation the information of a
sample of permutations $S$, and they can be seen as functions to compute the
\textit{mean} of sample $S$. The uBorda algorithm has been proposed to compute
a \textit{weighted mean} of sample $S$. UBorda has been show to return the
correct estimator with high probability on streaming contexts; in this paper,
we show its quality on black-box optimization problems. \ekhine{}{revisa esto
  por favor, Manuel}
...
This paper structured as follows. Section~\ref{sec:backgroud} summarizes fundamental concepts about permutation spaces, the uBorda algorithm, black-box combinatorial optimization under a limited budget and CEGO, a Bayesian optimizer for such problems. Section~\ref{sec:umm} presents the UMM algorithm, which is the main contribution of this paper. UMMA is analyzed using the experimental setup presented in Section~\ref{sec:setup} and results are presented in Section~\ref{sec:analysis}. We end the paper with overall conclusions and a call for further research on this type of optimization scenario (Section~\ref{sec:conclusions}).
...
Clearly, the choice of  $w(\sigma)$ depends on the relevant property of a ranking in a particular domain. For example, uBorda has been shown to have nice theoretical properties for the online estimation of the parameters of a sample with concept drift~\cite{irurozki2020}.
%
...
For the LOP, we generated synthetic instances as follows: sample 200 permutations from a MM of size $n=20$ and $\theta=-\log(uniformity)$, where  $uniformity=\{.5,.7,.9)\}$, i.e., from peaked distributions to almost uniform distributions. Then, generate matrix $a$ as $a_{i,j} = \sum_{\sigma\in S}\sum_{i<j}\mathbb{I}(\sigma(i)<\sigma(j))$.

Moreover, we consider real LOP datasets from the LOPLIB repository.\footnote{\url{http://grafo.etsii.urjc.es/optsicom/lolib/}}
In particular, we consider the following types: IO, Random instances of type A and Random instances of type B. 
...
Intel Xeon ``Ivybridge'' E5-2650v2 CPUs at 2.60\,GHz, 64\,GB RAM running CentOS Linux release 7.4.1708 (Core),
\ekhine{}{
Intel Xeon ``GenuineIntel'' E5-2680v3 CPUs at 2.50\,GHz, 32???\,GB RAM running Linux,
Intel Xeon ``GenuineIntel'' E5-2683v4 CPUs at 2.10\,GHz, 32???\,GB RAM running Linux.
}
%processor : 23
...
remarkably well.\MANUEL{Missing data, whitespaces where more than 6 hours of wall-clock time}  In particular, we observe that the pattern on
...
UMM. We elaborate this argument in the following sections with detail.\MANUEL{top and bottom}

Over all the instances  we
see that the configuration $r_1 =0.1$, $r_2=0.9$ has good
performance: \ekhine{}{the rank of this parameter configuration among all the possible configurations is less than 4 on average for all the instances}. From this
...
evaluations. Overall, \ekhine{UMM found  the best solutions in all the repetitions. }{the mean best solutions of UMM are better than the mean of CEGO regardless the uniformity of the instances. }
...
the fitness landscape. Results for larger PFSP corroborate these conclusions
...
random behaviour observed in some instances and discussed above,\ekhine{}{falta un cacho aqui}\MANUEL{Mío}
