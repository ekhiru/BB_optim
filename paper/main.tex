...
\renewcommand{\myparagraph}[1]{\smallskip{}\noindent\textbf{#1}.}
...
\myparagraph{CEGO} \EKHINE{tenemos una pagina entera dedicada a CEGO :s}\MANUEL{Sugiere qué quitar usando el commando \textbackslash{}ekhine}
...
As a consequence of this important distinction, when dealing with \manuel{data in the form of}{} permutations, in particular in 
...
\subsection{Experimental Analysis of UMM and CEGO} % on LOP and PFSP instances}
...
by the linear decrease in the expected distance\MANUEL{distance to the sampling model or variance of the sampling model?} of the sampling model, as described in Section~\ref{sec:sampling}. Overall, the rate of improvement
...
\myparagraph{Comparison of the best solutions found} We now turn our analysis
to the best (minimum) fitness found at the end of the run by each algorithm. We
show in Table~\ref{tab:results}
...
The computational complexity of UMM is \ekhine{}{ $O(n \log n)$ }at each iteration while both versions of CEGO need \manuel{tu sabes}{?}. This difference is evident in the time performance of the algorithms. The details can be seen in Table~\ref{table:time}.
