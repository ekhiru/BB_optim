...
\begin{algorithm}[t]
 ...
 \label{alg:umm}
  \begin{algorithmic}[1]
    ...
    \State $\Sigma \assign$ sampled $\beta$ permutations from  $MM(\hat\sigma_0, \theta)$\label{line:sample}\Comment{Generate $\beta$ permutations}
    \State $\sigma\assign  \arg\max_{\pi'\in \Sigma}\arg\min_{ \pi \in S} d(\pi', \pi)   $ \Comment{Select the farthest from $S$}
