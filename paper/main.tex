...
%150--250 words.
  ...
  typically several orders of magnitude smaller than in non-expensive
  problems. In this scenario, classical optimization methods such as
  mixed-integer programming and local search are not useful.  In the continuous
  ...
  black-box versions of LOP and PFSP show that UMM outperforms the solutions
  obtained by CEGO, a Bayesian optimization algorithm for combinatorial
  optimization. Nevertheless, a small change to CEGO proposed here,
  significantly improves its performance, thus producing solutions that are
  slightly better than those of UMM.
  % , specially when the budget of solutions evaluated approaches 400.
  As another benefit, the computational complexity of UMM increases linearly
  with both the number of function evaluations and the permutation size, which
  results in computation times an order of magnitude shorter than CEGO, which
  makes it specially useful when both computation time and number of
  evaluations are limited.  \sloppy
\end{abstract}
%
...
The detailed results of UMM$_{100}$  and UMM$_{200}$  and the original \CEGOorig are given in Table~\ref{tab:results_fit}. In this table, it can be seen that the quality of \CEGOorig  solutions is similar to the UMM$_{100}$ version of UMM. 
...
The computational complexity of UMM is $O(n \log n)$ at each iteration which causes the abrupt differencein the time performance of the algorithms. The details can be seen in Table~\ref{tab:results_time}. 
