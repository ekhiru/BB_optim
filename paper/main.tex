...
UMM \ekhine{}{has shown to perform statistically better or equal than CEGO, the
  state of the art algorithm, in the same instances as in the original paper of
...
evaluation. As shown by our results, a Bayesian optimizer such as CEGO may
sometimes require more than hour \MANUEL{UMM is able to model the LOP landscape
  at least as well as CEGO as shown by the plots where UMM shows a linear
  decrease in fitness.}  \EKHINE{me parece mas de CEGO q UMM y parece q UMM
  ...
  luego que addemás es más rápido} \MANUEL{Pero UMM es peor en rec13 y nug12, y
  ...
  entonces la diferencia en velocidad no importaría. } per fitness
evaluation. The time required to train the surrogate-model of CEGO also sharply
increases with permutation length, hence, UMM would be a computationally
