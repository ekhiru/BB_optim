...
evaluations are cheap and/or the fitness function is white-box.\MANUEL{Do EDAs really assume that the fitness function is white-box? ACO doesn't.}
...
Permutations are defined as bijections of the set $[n]$ of integers onto itself. The set of all permutations of $n$ values is denoted as $S_n$ and has cardinality $n!$. We represent permutations as an ordered vector and, we say that position $i$ contains value $j$ whenever $\sigma(i)=j$. We denote permutations with Greek letters.\MANUEL{Do we use $e$ anywhere? If not, we do not need this definition ``with exception of the identity permutation denoted as $e=(1, 2, 3, \dotsc,n)$.''} We denote the composition of $\sigma$ and $\pi$ as $\sigma\pi$ and the inverse of $\sigma$ as $\sigma^{-1}$, for which the relation $\sigma\sigma^{-1}=(1, 2, 3, \dotsc,n)$ always holds.
...
wall-clock time of 3 days for each run of CEGO. Hence, we set the budget of the
GA to $10^3$. Although a higher GA budget could improve the results of CEGO
reported here, our results are similar (and sometimes better) than the ones
reported in the original
papers~\citep{ZaeStoFriFisNauBar2014,ZaeStoBar2014:ppsn} due to running CEGO
with $\FEmax=400$ instead of $\FEmax=200$. In fact, we found that the increase
in runtime caused by doubling $\FEmax$ is much smaller than by increasing the
...
CPUs at 2.50\,GHz, both of them with 32\,GB RAM running Linux.\MANUEL{If you enter in a node and do \texttt{cat /etc/lsb-release} what does it say?} %
Experiments with
...
solutions. Lower uniformity values (e.g., \manuel{$\phi=.5$}{0.5})\MANUEL{We never use $\phi$, not even in the plots, so less have one less symbol to worry about.} show worsening evaluations
...
The distribution\MANUEL{what do you mean the distribution? Is this a feature of
  the matrix $a$?} of these instances differs from the synthetic LOP instances,
and, thus, so does the landscape. However, UMM shows a very similar behavior:
there is a (close to) linear decrease \manuel{on the function evaluations of
  new solutions along iterations}{of the fitness of new solutions over each new
  evaluation}. This suggests that UMM \manuel{very good fits}{is able to model}
the fitness landscape. Regarding CEGO, we \manuel{can take similar
  conclusions}{observe a similar behavior} for the first 50-100
\manuel{iterations}{evaluations}. At this point, CEGO reaches the time limit of
3 days, so no observations can be made for longer budgets.  Overall, the
improvement from the first (random) to the last solutions is 2-3 times larger
in UMM than CEGO. %
...
standard deviation) of the best solution found at the end of each run. We also report the mean runtime in hours and, since CEGO sometimes reached the limit of 72 hours, also the number of fitness evaluations reached by CEGO.

In the LOP, differences are mostly in favour of UMM, due to the time limit of
72 hours, which resulted in CEGO typically achieving less than 200
evaluations. In contrast, UMM was always able to reach 400 evaluations in less
than 16 hours. If UMM was allowed to reach the 72 hours limit, the differences
...
instances, CEGO reaches the time limit of 72 hours before consuming the budget
