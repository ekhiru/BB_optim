...
(EDAs)~\cite{Ceberio2013}.\MANUEL{X what would be the best work to cite about EDAs for
...
\end{equation}
%
and, the same as Borda, orders the \ekhine{items}{positions}\MANUEL{X why items? we talk about positions above. We could talk about decision variables if that is clearer\ekhine{}{id rather use the simplest possible notation. In statistical literature you ll see items/rankings. }} increasingly by their score.
...
\end{equation}
%
\MANUEL{X should the $\sigma$ be the same on both sides of $=$?}
...
A purely explotation approach would always generate $\sigma_0$ in the sampling stage. However, the setting of a decreasing variance \MANUEL{But we are controlling the expectation not the variance???\ekhine{}{both are a function of $\theta$}}  controls the exploration-explotation tradeoff as a function of the number of function evaluations that the algorithm performs.  Similar approaches have been taken in different contexts~\cite{ArzCebPer2019qap}.
...
experiments\MANUEL{\citet{ZaeStoBar2014:ppsn} claims this is called Swap
  distance, could you double-check?\ekhine{}{es la kendall among the inverses en realidad}}. Nevertheless,
...
\subsection{Setting the learning rate}
...
Over all the instances \MANUEL{X (which instances we tested? all synthetic?)} we
...
\EKHINE{cambio un poco al argumento}
...
to obtain competitive\EKHINE{hhmmm lo hacemos mejor o igual} results when compared with Bayesian optimizers. The
