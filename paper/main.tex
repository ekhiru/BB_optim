...
\EKHINE{6. When the main contribution of the paper is discussed it is stated that this contribution is the Unbalanced Mallows *model* (which does not seem to be true as the models used in the paper seem to be known already). Right after that it is stated that the contribution is *a novel algorithm* which, I think, reflects the contents of the paper much better.
}
In this work, we propose and analyze Unbalanced Mallows Model (UMM)\MANUEL{Perhaps we need to rename the algorithm to UMM Algorithm or UMM Optimizer to avoid the criticism of the reviewer that we do not propose the model?}, a
...
An equivalent definition for the Kendall's-$\tau$ distance counts the number of adjacent swaps that have to be made to convert $\sigma_1^{-1}$ into $\sigma_2^{-1}$ and, therefore, it is sometimes called \emph{swap distance}~\citep{ZaeStoBar2014:ppsn}. \EKHINE{ojo, aqui lo dice bien o  mal?}\MANUEL{Habría que mirarlo bien!}
...
\begin{algorithm}[t]
 ...
 \label{alg:umm}
  \begin{algorithmic}[1]
    \Require $\minit$: number of initial permutations, $\FEmax$: total budget,\newline\hspace*{5ex} \manuel{$\rho$: learning rate,}{$r_1, r_2$: parameters controlling the learning rate} 
...
Parameter $\theta$ is set automatically by the algorithm to control the diversification and intensification trade off. At the first iteration, $\theta$ is set so that  $\Edist = \binom{n}{4}$ (half of the expected distance for the uniform distribution),\MANUEL{Reviewer 3: "At the first iteration, $\theta$ is set so that $E[D] = {n \choose 4}$". Why?} and then the expected distance linearly decreases until the last iteration where $\theta$ is set such that $\Edist = 1$. Despite it is not possible to isolate $\theta$ in Eq.~\eqref{eq:expectation}, its monotonicity allows the use of bisection methods for efficiently finding an approximation. 

\EKHINE{5. "At the first iteration, Theta is set so that $E[D] = {n \choose 4}$". Why?}
...
samples $\minit=10$ solutions uniformly.  At each sampling step, we set
$\beta=10$, meaning that 10 permutations are generated and the farthest from
$S$ is selected for evaluation.  The other two parameters of UMM are $r_1$ and
$r_2$, which determine the balance between exploration and exploitation. In the
experimental section we will study their impact.
...
%
Experiments with real-world expensive black-box problems would be computationally infeasible. Instead, we consider classical combinatorial optimization problems as black-box optimization problems. \ekhine{cambio la definition de LOP}{In this section, and following the consensus on the optimization literature, the permutations denote orderings. }\MANUEL{That seems strange, shouldn't be use the same everywhere? I would rather use rankings or orderings everywhere and I have a slight preference for orders since this is an optimization paper.}
...
over 10 runs with different random seeds on the same problem instance.  As an
example, each line in Fig.~\ref{fig:lop_synth} shows the mean fitness of the
\ekhine{solutions evaluated by either UMM or CEGO on one synthetic LOP instance}{ojo}. The
shaded area shows one standard deviation around the mean. Ideally, each new
evaluation will monotonically improve in fitness, since each solution evaluated
helps to refine the model and leads to a better solution being evaluated in the
next step.  However, it is possible that new solutions are worse
...
\begin{table*}[tb] % resumns
\begin{tabular}{l@{\hskip -2ex}*{5}{r}rl@{\hskip -2ex}*{3}{r}}%llrlrllrrr}
\toprule
%Problem &    Instance &  CEGO\_Fitness\_mean & CEGO\_Fitness\_std &  UMM\_Fitness\_mean & UMM\_Fitness\_std &    Confidence interval &  CEGO F. evaluations &  CEGO Runtime &  UMM Runtime \\
            ...
            \bf Problem & \bf     Instance & \mcolc{2}{CEGO} & \mcolc{2}{UMM}   &                         &            & \bf & \bf CEGO       & \bf UMM                       \\\midrule
\midrule
    LOP &    N-p40-01 &            10496.6 &          (116.8) &           12369.1 &         (389.9) &     [-2157.1, -1587.9] &                400.0 &          19.9 &          0.3 \\
    LOP &    N-p40-02 &            11714.7 &           (84.7) &           13160.5 &         (454.8) &     [-1773.5, -1118.1] &                400.0 &          20.1 &          0.3 \\
    LOP &  N-t59b11xx &           102408.0 &         (3800.2) &          114075.5 &        (5389.2) &    [-16084.2, -7250.8] &                400.0 &          21.1 &          0.4 \\
    LOP &  N-t59d11xx &            12692.2 &         (1097.9) &           16662.9 &        (2155.3) &     [-5618.5, -2322.9] &                400.0 &          21.2 &          0.4 \\
    LOP &    N-p50-02 &            19090.5 &          (285.4) &           21746.6 &         (645.4) &     [-3140.6, -2171.6] &                400.0 &          22.9 &          0.5 \\
    LOP &    N-p50-01 &            18197.7 &          (225.6) &           21161.8 &         (657.4) &     [-3447.4, -2480.8] &                400.0 &          23.2 &          0.5 \\
    LOP &  N-sgb75.01 &          1893533.2 &        (28594.6) &         1992059.9 &       (40235.7) &  [-131577.1, -65476.3] &                400.0 &          33.2 &          1.0 \\
    LOP &  N-sgb75.02 &          1895801.8 &        (82847.9) &         1967132.3 &       (49228.1) &   [-136421.2, -6239.8] &                400.0 &          33.3 &          1.1 \\
   PFSP &       rec05 &             1276.2 &           (15.1) &            1276.3 &           (8.4) &          [-11.8, 11.6] &                400.0 &          18.3 &          0.1 \\
   PFSP &       rec13 &             1992.9 &           (27.6) &            2057.8 &          (22.8) &         [-88.8, -41.0] &                400.0 &          18.5 &          0.1 \\
   PFSP &       rec19 &             2264.5 &           (45.0) &            2304.8 &          (37.7) &          [-79.4, -1.2] &                400.0 &          18.2 &          0.2 \\
   PFSP &       rec31 &             3415.5 &           (62.1) &            3419.3 &          (37.0) &          [-52.6, 45.0] &                400.0 &          23.1 &          0.5 \\
\bottomrule
\end{tabular} 
\end{table*}
...
\toprule
Problem &    Instance &  CEGO\_inv &  CEGO\_orig &  UMM\_100 &  UMM\_200 &  UMM\_400 &  UMM\_no\_inv \\
\midrule
    LOP &  N-t59b11xx &   1265.98 &    1240.36 &     1.37 &     5.78 &    22.24 &       21.89 \\
    LOP &  N-t59d11xx &   1269.93 &    1252.89 &     1.37 &     5.62 &    21.84 &       22.30 \\
    LOP &    N-p40-01 &   1195.90 &    1170.81 &     1.14 &     4.61 &    18.13 &       19.06 \\
    LOP &    N-p40-02 &   1207.24 &    1183.09 &     1.15 &     4.76 &    18.11 &       19.06 \\
    LOP &    N-p50-01 &   1391.55 &        NaN &      NaN &      NaN &    27.36 &         NaN \\
    LOP &    N-p50-02 &   1372.71 &        NaN &      NaN &      NaN &    28.36 &         NaN \\
    LOP &  N-sgb75.01 &   1994.22 &    2004.70 &     3.97 &    15.77 &    62.75 &       65.40 \\
    LOP &  N-sgb75.02 &   1995.09 &    1975.41 &     4.03 &    15.90 &    64.58 &       62.69 \\
   PFSP &       rec05 &   1099.63 &     917.81 &     0.30 &     1.14 &     4.54 &        4.53 \\
   PFSP &       rec13 &   1109.51 &    1043.08 &     0.31 &     1.07 &     4.55 &        4.50 \\
   PFSP &       rec19 &   1090.06 &    1087.38 &     0.63 &     2.59 &    10.51 &       10.35 \\
   PFSP &       rec31 &   1387.09 &    1367.26 &     1.79 &     6.90 &    27.84 &       26.91 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
    ...
    LOP &    N-p50-02 &           19090.5 &                NaN &          21746.6 &              NaN &              NaN &                 NaN \\
    LOP &  N-sgb75.01 &         1893533.2 &          2095899.6 &        1992059.9 &        2011271.7 &        2074111.6 &           2111368.5 \\
    LOP &  N-sgb75.02 &         1895801.8 &          2069286.9 &        1967132.3 &        2011004.2 &        2045556.5 &           2056681.8 \\
   PFSP &       rec05 &            1276.2 &             1318.7 &           1276.3 &           1283.1 &           1300.4 &              1332.5 \\
   PFSP &       rec13 &            1992.9 &             2096.0 &           2057.8 &           2113.6 &           2134.9 &              2163.6 \\
   PFSP &       rec19 &            2264.5 &             2328.4 &           2304.8 &           2346.5 &           2392.4 &              2405.2 \\
   PFSP &       rec31 &            3415.5 &             3512.3 &           3419.3 &           3438.9 &           3511.9 &              3554.1 \\
\bottomrule
\end{tabular}
\end{table*}
...
\ekhine{probar con m=100 para ver ci podemos competir en instancias cortas}{}
