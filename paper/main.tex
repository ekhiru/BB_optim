...
%150--250 words.
  ...
  model and unbalanced rank aggregation (uBorda).
  ...
  version.  As another benefit, the computational complexity of UMM increases
  linearly with both the number of function evaluations and the permutation
  size, which results in computation times an order of magnitude shorter than
  CEGO, which makes it specially useful when both computation time and number
  of evaluations are limited.  \sloppy
\end{abstract}
%
...
CEGO~\citep{ZaeStoFriFisNauBar2014,ZaeStoBar2014:ppsn}, the surrogate model is optimized by a GA with  population size of 20, crossover rate of 0.5,
...
GA with a budget of $10^4$ evaluations of the model as a surrogate of the true
fitness function.  The original paper~\citep[p.~875]{ZaeStoFriFisNauBar2014} notes that coupling the GA with local
search does not improve the results significantly since the model is anyway an
inexact estimation of the original fitness
function.
...
%
Table~\ref{tab:results} also shows the runtime (in hours) of \CEGOinv and UMM when both
