...
Permutation $\sigma$ represents a ranking of item:  $\sigma(i)=j$ denotes that item $i$ has rank $j$. The ordering associated with $\sigma$ is given by its inverse $\sigma^{-1}$, therefore,  $\sigma(i)=j \Leftrightarrow  \sigma^{-1}(j)=i$. This distinction is in order since statistical methodologies are devoted for rankings and optimizaiton literature for orderings and mixing both concepts is a very common mistake. We will illustrate in the experimental section the consequences of this confusion. 

Kendall's-$\tau$ distance is the metric for rankings that counts the pairs of items ranked in different order in both permutations:
...
An equivalent definition for the Kendall's-$\tau$ distance counts the number of adjacent swaps that have to be made to convert $\sigma_1^{-1}$ into $\sigma_2^{-1}$ and, therefore, it is sometimes called \emph{swap distance}~\citep{ZaeStoBar2014:ppsn}. \ekhine{ojo, aqui lo dice bien o  mal?}{}
...
\begin{algorithm}[t]
 ...
 \label{alg:umm}
  \begin{algorithmic}[1]
    ...
    \State $\Sigma \assign$ sample $\beta$ permutations from  $MM(\hat\sigma_0, \theta)$\label{line:sample}\Comment{Generate $\beta$ permutations}
    \State $\sigma\assign  \argmax_{\pi'\in \Sigma}\min_{ \pi \in S} d(\pi', \pi)   $ \Comment{Select the farthest from $S$}\label{line:distance}
...
the weights is concentrated in the best $100r_2$\% of the solutions in $S$, i.e.,\MANUEL{Reviewer 3:  Justify the form of the formula used to learn the "learning rate" $\rho$ (equation 4). Why this particular approach was chosen?}
%
\begin{equation}
  \begin{split}
...
\ekhine{4. Justify the form of the formula used to learn the "learning rate" $\rho$ (equation 4). Why this particular approach was chosen?}{}
...
\subsubsection{CEGO} \ekhine{tenemos una pagina entera dedicada a CEGO :s}{}
...
\bibliographystyle{splncs04natnodoi}
