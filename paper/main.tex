...
authors\footnote{\url{https://cran.r-project.org/package=CEGO}}. Following the authors of CEGO~\citep{ZaeStoFriFisNauBar2014,ZaeStoBar2014:ppsn}, we use and the GA that optimizes
...
the GA is \textcolor{red}{$10^4$} evaluations using the
...
function and the result is used to build the initial surrogate model.  Here we
use $\minit=20$.\MANUEL{\citet{ZaeStoBar2014:ppsn} uses 10 so we may want to change this}
...
\manuel{Computing system details: MANUEL}{%
  All experiments were run on Intel Xeon E5-2640 CPUs at 2.50\,GHz, 64\,GB RAM
